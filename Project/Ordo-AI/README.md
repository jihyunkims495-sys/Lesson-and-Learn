...
**Version 1.0.0 · 2026-09-19 · Daytona submission candidate**

[Project Index](../README.md) · [Public Demo](https://ordo-ai-decision-os-jihyu.jihyunkims495.chatgpt.site) · [Version History](./CHANGELOG.md) · [Final Release Record](./docs/FINAL_RELEASE.md)
...
- 포함: Entry, Today, Overview, Decisions, Simulator, Reports, Calendar와 Quick Guide
...
- 제외: 라이브 Supabase, 운영 주문·재고 API, 외부 AI 추론, 실제 발주 전송, Daytona Sandbox 실행
...
- 수치 계산은 재현 가능한 규칙 기반 로직이 담당하고, AI는 검증된 결과의 해석·요약·리스크 설명을 담당하도록 분리했습니다.
...
현재 버전은 의사결정 경험을 검증하기 위한 정적 데모입니다. 실시간 주문 수집, 운영 Supabase 연결, 외부 AI 추론, 실제 PO 전송, 예측 백테스트는 아직 연결되지 않았습니다.

## Daytona 적용 계획

Daytona는 계산 결과를 꾸미기 위한 요소가 아니라, 동일한 입력과 엔진 버전으로 의사결정을 안전하게 재현하는 격리 실행 환경으로 적용할 계획입니다.
...
→ Daytona Sandbox 생성
→ 버전이 고정된 리오더 엔진 실행
→ validation.json / metrics.json / recommendations.csv / run.log 반환
→ AI 설명 생성
→ MD 승인
```

수량과 KPI는 결정론적 계산으로 산출하고, AI는 그 결과를 설명합니다. 실행별 입력 해시, 엔진 버전과 산출물을 보존해 같은 조건에서 같은 판단을 재현할 수 있도록 확장할 예정입니다.
...
- Planned isolated execution: Daytona Sandbox
- Deployment: static public preview

## 공개 데모의 데이터 경계

화면에 표시되는 주문, 예측, 환율, 추천과 판단 결과는 실제 운영 구조를 모사한 결정론적 데모 데이터입니다. 현재 공개 버전은 라이브 데이터베이스, 운영 스케줄러, 외부 AI 서비스 또는 실제 발주 전송이 연결된 제품이라고 주장하지 않습니다.
