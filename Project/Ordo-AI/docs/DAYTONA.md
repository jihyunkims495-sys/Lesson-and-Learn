...
Daytona DEV URL은 지정된 시간 동안 열리는 독립 시연 경로입니다. 링크를 공개해도 해당 포트의 시연 화면만 접근할 수 있으며, 관리 API 키나 샌드박스 전체 접근 토큰은 포함되지 않습니다.

## 2. Daytona를 사용한 부분

1. 전용 샌드박스 `ordo-hackathon-20260919`에서 Node.js 프로세스를 실행했습니다.
2. 정적 앱·폰트·영상·합성 데이터와 계산 엔진을 전용 디렉터리에 업로드했습니다.
...
단순히 스폰서 이름이나 링크만 추가한 것이 아닙니다. 다만 화면의 모든 계산을 서버로 이전한 것도 아닙니다. 클라이언트 계산은 유지하고, 명시적인 서버 실행 버튼을 추가한 범위입니다.
