...
function openPanel(type){
 ...
 const content={how:`<div class="how-guide"><div class="eyebrow">QUICK GUIDE</div><h2>How to use</h2><section class="drawer-section"><h3>판매 분석</h3><p>일별 매출과 재고 흐름을 한눈에 확인해요.</p></section><section class="drawer-section"><h3>발주 시뮬레이션</h3><p>상품·옵션별 수량과 세 가지 가정을 비교해요.</p></section><section class="drawer-section"><h3>결정·리스크 추적</h3><p>판단과 위험을 기록해요. 실제 발주는 없어요.</p></section><div class="how-final-order"><h3>최종 물량 발주</h3><p>Ordo AI와 최적의 물량을 결정해요.</p></div></div>`,calendar:brandCalendar(),data:window.OrdoWorkbench.dataPanel(),admin:`<div class="admin-menu" aria-label="관리 메뉴"><button type="button" class="entry-pill admin-menu-button">MASTER ADMIN</button><button type="button" class="entry-pill admin-menu-button">BigQuery</button><button type="button" class="entry-pill admin-menu-button">CLIENT PAGE</button></div>`,search:`<div class="eyebrow">GLOBAL SEARCH</div><h2>Search</h2><input class="search-input" id="search-input" placeholder="상품 코드 또는 화면 검색" autofocus><div class="drawer-section" id="search-results"><b>QUICK ACCESS</b><p>Today · Overview · Decisions · Simulator · Reports</p></div>`};
 const drawer=document.querySelector('#drawer');document.querySelector('#drawer-content').innerHTML=content[type]||'';drawer.classList.toggle('calendar-drawer',type==='calendar');drawer.classList.add('open');document.querySelector('#panel-backdrop').classList.add('open');drawer.setAttribute('aria-hidden','false');
...
function closePanel(){window.OrdoCalendar?.destroy();const d=document.querySelector('#drawer');d.classList.remove('open','calendar-drawer');document.querySelector('#panel-backdrop').classList.remove('open');d.setAttribute('aria-hidden','true')}
