...
(function () {
  ...
  function notice() {
    return `<div class="wb-notice"><b>${BUILD}</b><span>${snapshot.calibration?'DEV 보정 샘플':'DEV 합성 데이터'} ${esc(snapshot.meta.asOf)} · 관측 / 추정 / MD 기록 분리</span><button data-panel="data">소스·한계 보기</button><button data-wb-refresh>데이터 재확인</button><span class="wb-live-status" data-wb-last-run>마지막 검사 ${ledger.lastRun ? new Date(ledger.lastRun).toLocaleTimeString('ko-KR') : '준비 중'}</span>${policyFailure?`<strong class="wb-danger">${esc(policyFailure)}</strong>`:''}${storageFailure ? `<strong class="wb-danger">${esc(storageFailure)}</strong>` : ''}</div>`;
  }
  ...
  function forecastChart(portfolio,chartModel=model) {
    ...
    return explanation+window.OrdoCharts.render([...history,...portfolio.daily],{id:'overview-'+state.horizon,context:chartModel.meta.runtimeFingerprint,fullPeriod:true,title:'관측에서 미래까지 · 일별 매출',expected:true,baseline:true,observedThrough:snapshot.meta.asOf});
  }
  function overview() {
  ...
  function dailyGuidelines() {
    ...
    return `<section class="wb-panel wb-daily-guidelines"><div class="section-title"><h2>주간 판매 비중</h2><div class="segments"><button data-wb-guideline="applied" class="${state.guidelineTab==='applied'?'active':''}">현재 현황</button><button data-wb-guideline="edit" class="${state.guidelineTab==='edit'?'active':''}">판매 비중 시뮬</button></div></div><p>주간 판매가 아래 요일 비중으로 배분됩니다. 월·금 상승, 화–목 낮음, 주말 피크를 반영한 샘플입니다.</p>${window.OrdoWeeklySales.render(snapshot,model.meta.effectiveWeekdayFactors)}<div class="wb-weekdays">${labels.map((label,i)=>`<div><b>${label}</b>${state.guidelineTab==='edit'?`<label><span class="sr-only">${label}요일 프로모션 판매 수요</span><input data-wb-promotion="${i}" type="number" min="0" max="1000000" step="1" value="${draft[i]}"></label><small>${total?(draft[i]/total*100).toFixed(1):'0.0'}%</small>`:`<strong>${(profile.shares[i]*100).toFixed(1)}%</strong>`}</div>`).join('')}</div>${state.guidelineTab==='edit'?`<p>프로모션 예상 판매 수량을 요일별로 입력하세요. 적용 기간 ${next[0]?.date||'—'}–${next.at(-1)?.date||'—'} · 이 기간 밖의 수요와 원본 주문은 변경하지 않습니다. 실제 판매량은 옵션 재고 한도까지이며 초과 수요는 결품 손실입니다.</p><div class="wb-actionbar"><button class="btn orange-btn" data-wb-promotion-apply>수동 판매 수량 적용</button><button class="btn ghost" data-wb-promotion-reset>기본 현황 복원</button></div>`:''}${state.promotionUnits?'<p class="wb-warning">판매 비중 시뮬 적용 중 · 아래 4개 지표에 기본 현황 대비 차이를 표시합니다. 이 시험은 Overview에만 적용되며 발주 검토·확정 기록은 바꾸지 않습니다.</p>':''}</section>`;
  }
  function filtered() {
  ...
  function decisions() {
    ...
    return `<section class="page wb-page">${title('DECISIONS / REVIEW & DECIDE','전체 상품을 분류하고,<br>최종 결정은 직접 내리세요.',scopeName+' 기준 · 분류와 MD 결정은 별도 기록',tabs)}${notice()}
