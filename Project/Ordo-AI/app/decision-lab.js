(function () {
  ...
  function notice() {
    return `<div class="source-notice"><b>${snapshot().calibration?'DEV 보정 샘플':'DEV MOCK'} · ${esc(snapshot().meta.asOf)} 기준</b><span>원본 샘플 주문 + DEV 생성 주문 / 재고·초도 집행액은 별도 DEV 가정 · 외부 LLM·실제 발주·외부 자동 수집 미연결</span><button data-panel="data">데이터 한계 ↗</button></div>`;
  }
  ...
  function simulator(state, model, ledger = {}) {
    ...
    return `<section class="page audited-page lab-page" data-lab-view="simulator">${header('SIMULATOR / PRODUCT DECISION LAB', '수량의 근거를 보고,<br>MD가 최종 판단합니다.', '현재 권고는 전체 예산 배분 후 판단 · 아래 카드는 선택 상품의 수량·수요 가정 비교', productSelect(product))}${notice()}<div class="lab-container">${productBanner(product)}
  ...
  function dailyFeedPanel() {
    ...
    return `<div class="lab-next-week"><b>다음 주 · ${week?.from||'—'}–${week?.to||'—'}</b><span>${week?.events.length?week.events.map(e=>esc(e.date.slice(5))+' '+esc(e.title)).join(' / '):'캘린더에 등록된 주요 일정이 없습니다.'}</span><small>메인 캘린더 기준</small></div><section class="lab-card lab-dev-feed"><div class="lab-section-heading"><div><span class="mono-label">DAILY DEV ORDER FEED / 가상 주문</span><h2>${s.meta.asOf}까지 반영</h2></div></div><p>매일 KST 08:00 이후 전일 주문을 자동 생성합니다. 페이지가 닫혀 있던 날은 다음 접속·재점검 때 빠짐없이 보충합니다. 같은 날짜는 중복 생성하지 않습니다. 수동 수집은 한국 시간 기준 전날까지만 가능합니다. 오늘·미래 주문은 생성하지 않습니다. 실제 외부 주문이 아닌 DEV 가상 주문입니다.</p><p class="lab-feed-status">${last?`최근 배치 ${last.date} · ${number(last.rows)}건 / ${number(last.units)}개 / ${won(last.revenue)} · 누적 ${feed.batches.length}일`: '아직 추가 배치가 없습니다.'} · 원본 ${s.meta.sourceAsOf||s.meta.asOf}</p>${window.OrdoDevFeed?.warning?`<p class="lab-blocked">${esc(window.OrdoDevFeed.warning)}</p>`:''}<details><summary>주문 원장 · ${feed?.rows||0}건 (최근 200건 표시)</summary><button class="btn ghost" data-wb-dev-export>전체 주문 CSV</button><div class="lab-table-scroll"><table class="lab-table"><thead><tr><th>주문 ID / 날짜</th><th>상품·옵션</th><th>수량</th><th>매출</th></tr></thead><tbody>${(feed?.orders||[]).slice(-200).reverse().map(o=>`<tr><td>${esc(o.id)}<small>${o.date} · DEV 생성</small></td><td>${esc(o.code)}<small>${esc(o.optionId)}</small></td><td>${number(o.units)}</td><td>${won(o.revenue)}</td></tr>`).join('')}</tbody></table></div></details><small>자동 데이터 기간은 현재 FW 샘플 시즌 내입니다. 외부 주문 수집·상시 서버 스케줄러와는 별개이며 생성 결과는 적중률 검증 자료가 아닙니다.</small></section>`;
  }
